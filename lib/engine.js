          ...
          if (signal === 'buy') {
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct) {
                ...
                //}
                //else {
                  console.log('\nplacing sell order at ' + fc(price) + ', ' + fc(Number(price) - quote.ask) + ' over best ask\n')
                  if (c.xmppon) c.xmpp.send(c.xmppto,'placing sell order at ' + fc(price) + ', ' + fc(Number(price) - quote.ask) + ' over best ask\n')
