            ...
            s.exchange.getOrder({order_id: order.order_id, product_id: s.product_id}, function (err, api_order) {
              ...
              if (new Date().getTime() - order.local_time >= so.order_adjust_time) {
                getQuote(function (err, quote) {
                  ...
                  var marked_price
                  if (type === 'buy') {
                    marked_price = n(quote.ask).subtract(n(quote.ask).multiply(so.markup_pct / 100)).format(s.product.increment, Math.floor)
                    ...
                    marked_price = n(quote.bid).add(n(quote.bid).multiply(so.markup_pct / 100)).format(s.product.increment, Math.ceil)
                    if (n(order.price).value() > marked_price) {
                      msg(marked_price + ' vs our ' + order.price)
                      cancelOrder(true)
                    }
    ...
    function executeSignal (signal, _cb, size, is_reorder, is_taker) {
      ...
      syncBalance(function (err) {
        ...
        getQuote(function (err, quote) {
          ...
          if (signal === 'buy') {
            price = n(quote.ask).subtract(n(quote.ask).multiply(so.markup_pct / 100)).format(s.product.increment, Math.floor)
            if (!size) {
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || ('min_total' in s.product && s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              //}
              //else {
                console.log('\nplacing buy order at ' + fc(price) + ', ' + fc(quote.ask - Number(price)) + ' under best ask\n')
                
                if (c.xmppon) c.xmpp.send(c.xmppto,'placing buy order at ' + fc(price) + ', ' + fc(quote.ask - Number(price)) + ' under best ask\n')
            ...
            if ((s.product.min_size && Number(size) >= Number(s.product.min_size)) || (s.product.min_total && n(size).multiply(price).value() >= Number(s.product.min_total))) {
              ...
              if (so.max_sell_loss_pct && sell_loss > so.max_sell_loss_pct) {
                ...
                //}
                //else {
                  console.log('\nplacing sell order at ' + fc(price) + ', ' + fc(Number(price) - quote.bid) + ' over best bid\n')
                  if (c.xmppon) c.xmpp.send(c.xmppto,'placing sell order at ' + fc(price) + ', ' + fc(Number(price) - quote.bid) + ' over best bid\n')
